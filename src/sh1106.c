...
static void SH1106_cmd_double(uint8_t cmd1, uint8_t cmd2) {
  ...
  command[0] = cmd1;
  command[1] = cmd2;
  spi_write_blocking(SH1106_SPI_PORT, command, 2);
}

// Initialize SPI peripheral and SH1106 display
void SH1106_Init(void) {
  spi_init(SH1106_SPI_PORT, SH1106_SPI_BAUD);

	gpio_set_function(SH1106_GPIO_SCK, GPIO_FUNC_SPI);
	gpio_set_function(SH1106_GPIO_MOSI, GPIO_FUNC_SPI);
  gpio_init(SH1106_GPIO_DC);
  gpio_init(SH1106_GPIO_RST);
  gpio_init(SH1106_GPIO_CS);
	gpio_set_dir(SH1106_GPIO_DC, GPIO_OUT);
	gpio_set_dir(SH1106_GPIO_RST, GPIO_OUT);
	gpio_set_dir(SH1106_GPIO_CS, GPIO_OUT);

	gpio_put(SH1106_GPIO_CS, 1);
	gpio_put(SH1106_GPIO_DC, 1);
	gpio_put(SH1106_GPIO_RST, 1);
...
void SH1106_Flush(void) {
  ...
  for (register uint32_t page = 0; page < bits_h; ++page) {
    ...
    spi_write_blocking(SH1106_SPI_PORT, (uint8_t *)ram_pointer,
                       sizeof(ram_pointer));
    uint8_t *vram = &vRAM[page * SCR_W];
    SH1106_DC_H();
    spi_write_blocking(SH1106_SPI_PORT, vram, SCR_W);
    SH1106_DC_L();
  }
  SH1106_CS_H();
}
